...
L<version  Markdown|https://github.com/jforget/Perl-fixed-width-char-human-recognition/blob/master/description/description.md>
de ce  document. En  effet, au  printemps 2022,  j'ai remarqué  que le
rendu  de la  version POD  par Github  posait des  problèmes dans  les
paragraphes comportant  des caractères au-delà de  U+00FF. C'est ainsi
que dans la version POD, j'ai dû écrire S<« oeuvre »> sans lier le
S<« o »>  et le S<« e »>  et que j'ai dû dessiner des  flèches avec un
tiret  et un  signe supérieur  S<ainsi :>  S<« -E<gt>  »>. La  version
Markdown ne présente pas ces anomalies.
...
S<Remarque :> ultérieurement, j'ai retrouvé un quatrième listing, daté du 12 décembre 1983.
...
shell basé sur le schéma S<ci-dessous :>
...
I<Copie d'écran personnelle d'une page 
...
I<La Liseuse, par Fragonard (1732--1806), oeuvre dans le domaine public, avec quelques adaptations personnelles mineures.>
...
I<Source : En classe, le travail des petits, par Henri Jules Geoffroy (1853--1924), oeuvre dans le domaine public. Avec un marquage par le site web où j'ai trouvé cette reproduction (pourquoi marquer une oeuvre dans le domaine S<public ?)>>
...
I<Source L<http://www.popscreen.com/p/MTU3MDkzMDU5/Soviet-CCCP-Astronaut-Space-Patch-Russian-Alexei-Leonov>>

la plupart des systèmes donnent comme résultat S<« cécécépé »> (U+0043 U+0043 U+0043 U+0050),
...
I<S<Source :> copie d'écran personnelle du logiciel L<Stellarium|http://www.stellarium.org/>>
...
I<S<Source :> dessin créé par L<GEF|http://www.gef.free.fr/ambigallery.html> utilisé ici avec l'autorisation des intéressés.>
...
était prévue pour afficher le texte résultat.

=for HTML
<img src='Alice1.png' alt='Première étape de la reconnaissance de caractères par Alice-OCR' />

I<Source : Portrait de Lena Söderberg, (c) Playboy, 1972, utilisé avec la permission tacite de Playboy, incorporé dans une copie d'écran personnelle>
...
=for HTML
<img src='Alice2.png' alt='Deuxième étape de la reconnaissance de caractères par Alice-OCR : reconnaissance automatisée' />
...
=for HTML
<img src='Alice3.png' alt='Troisième étape de la reconnaissance de caractères par Alice-OCR : reconnaissance par un humain' />
...
Les associations glyphe -E<gt> caractère
pouvaient même être stockées dans
une base de données pour l'analyse
d'un autre fichier.

S<Remarques :> en 2000 ou 2001, j'ai fait très peu d'essais, car j'ai rapidement conclu
...
=item * Parce que cela montre sans ambiguïté qu'il s'agit d'un fichier graphique et non
d'un fichier texte,

=item * Parce que la tradition veut que tout programme élémentaire affiche S<« Hello
World ! »>, que toute leçon de programmation présentant la récursivité prenne pour exemples la factorielle
...
J'ai essayé de soumettre le portrait de Lena (avec la bulle S<« Hello World ! »>) à C<gocr>
et à C<cuneiform>. Ils s'en sont tirés correctement. Comme quoi, ce sont quand même de bons programmes 
...
point. Dans le vrai Alice-OCR, c'était le cas avec les S<« i »>, je ne me souviens pas
avoir testé avec des fichiers graphiques contenant des S<« ! »>, des  S<« j »>, des lettres
...
=head3 Autres programmes hybrides
...
L<un autre projet|https://github.com/jforget/raku-Hamilton2/blob/master/doc/Hamilton.fr.md>,
j'ai utilisé le module GD.pm de Perl 5 avec Raku et avec le module 
L<Inline::Perl5|https://modules.raku.org/dist/Inline::Perl5:cpan:NINE> de Raku.
Peut-être aurais-je pu faire de même en S<2017 ?>
...
I<Copie d'écran personnelle d'une page Wikipedia avec des modifications mineures. Comme indiqué dans la page Wikipedia,
...
=for HTML
<img src="r.png" alt="Cinq exemples de 'r' pris dans le listing" />
...
Exemple tiré du réel, le même caractère S<« p »> que ci-dessus, traité par un
lissage à 8 voisins.

=for HTML
<img src="resultat-lissage-v8.png" alt="Extrait d'un listing avant et après lissage, avec au milieu la visualisation des différences" />
...
=head2 Conclusion partielle

Après coup, je me rends compte que le lissage des glyphes ne sert à rien. Comme vous pouvez le voir
sur l'exemple ci-dessus, cela permet de changer un nombre très réduit de S<pixels :> 4 dans l'exemple
...
atteindre la centaine. Donc, I<a posteriori>, le jeu n'en valait pas la chandelle.
...
de fichiers S<graphiques :> une base de données pour les listings sortant d'une 
...
J'ai utilisé le terme S<« appli »>. J'aurais peut-être dû appeler cela S<« projet »>
ou utiliser un autre nom. C'est  S<« appli »>, c'est comme ça.

=head2 Le modèle logique des données

Le schéma de base basique, qui sera complété ultérieurement, S<est :>
...
pour les caractères espace à S<« } ».> Je rappelle
...
qu'à la notion de S<« quelque-chose> qui peut se stocker dans une base de 
S<données »>, j'ai préféré adopter un autre nom pour cette table. Le 
nom adopté est S<« Coloriage »>, parce que le résultat de la requête comporte un fichier
...
complètement pour le mot S<« grégorien »>

=for HTML
<img src="decalage-grille.png" alt="Décalage en fin de ligne" />

L'idée consiste à introduire une nouvelle notion de Grille et considérer qu'un Document se découpe
en plusieurs Grilles, lesquelles se découpent en Cellules. Le modèle de données devient S<donc :>
...
=head3 Découpage en plusieurs grilles
...
la version initiale du schéma de S<données :>
...
Il y a un cas particulier, la Grille avec la Cellule C<l = 0, c = 0> en haut à gauche.
...
Pour les autres Grilles, la création se fait en saisissant les coordonnées C<l, c> du 
...
S<« gregorien »>, l'utilisateur a déterminé que la Grille par défaut S<est :>

  l   c  prio    x    y  lg  ht action
  0   0    0    42   27  30  49

L'utilisateur décide de créer une deuxième Grille pour le mot S<« gregorien »>. Le numéro de ligne
est à coup sûr 3, mais combien vaut le numéro de S<colonne ?> Sans faire attention à la ligne verticale
en vert foncé, l'utilisateur essaie au S<pif :>
...
Le programme réaffiche S<alors :>
...
Voici ce que cela donne S<avec :>

=begin HTML

<img src="Liste-des-Grilles.png" alt="Liste des Grilles" />

<img src="recalage-grille.png" alt="Recalage des Cellules en fin de ligne" />

=end HTML

On remarquera qu'il reste des pixels rouges dans S<« gregorien »>. Ce n'est
pas dû à la nouvelle Grille, celle qui chapeaute le caractère S<« g »>, mais à l'ancienne
...
Et cela s'est passé dans mes tests. J'ai créé quelques Glyphes pour la lettre S<« l »>  et le
chiffre S<« 1 »>. En faisant passer l'association entre Cellules et Glyphes, l'une des Cellules
contenant une lettre S<« l »> s'est retrouvée associée à un Glyphe chiffre S<« 1 »>.

=for HTML
<img src="probleme1.png" alt="Association d'une Cellule lettre l avec un Glyphe chiffre 1" />

Le score est de 47, dont 8 pour la barre verticale, 11 pour le pied et 28 pour la goutte d'encre
en haut à gauche, censée faire la différence entre les lettres S<« l »> et les chiffres S<« 1 »>.

La comparaison avec les Glyphes lettres S<« l »> S<donne :>

=for HTML
<img src="probleme2.png" alt="Association d'une Cellule lettre l avec un Glyphe lettre l" />

=for HTML
<img src="probleme3.png" alt="Association d'une Cellule lettre l avec un Glyphe lettre l" />
...
du glyphe S<« 1 »>. Pour le premier Glyphe S<« l »>, c'est un peu normal, la barre verticale de la
Cellule est large de 4 pixels alors que la barre verticale du Glyphe fait 5 pixels de large et il
faut donc effacer une trentaine de pixels. Mais pour le second Glyphe S<« l »>, la barre
...
en nombre S<entier ?> Comment arrondir les coordonnées xg et yg ?

La solution adoptée consiste à faire plusieurs essais, en arrondissant par défaut
puis par excès. Pour les abscises xg_Cel et xg_Gly, on fait une série de tests
où les deux sont arrondies par défaut, une deuxième où xg_Cel est arrondie par
excès tandis que xg_Gly est arrondie par défaut, puis une troisième où
xg_Cel est de nouveau arrondie par défaut et xg_Gly est cette fois-ci arrondie
...
de pixels différents entre la Cellule et le S<Glyphe ?>
...
n'a aucun pixel noir avec S<< x < 3 >> ni avec S<< y > 4 >>.  De même, le Glyphe n'a aucun
pixel noir avec S<< x > 6 >> ni avec S<< y < 2 >>. Donc, les deux boucles emboîtées
...
Le score est donc S<2 + 3 = 5>. Avec la seconde méthode, la Cellule a 8 pixels noirs et le Glyphe 
en a 9. Le calcul commence donc avec un score de 17. Comme il y a 6 pixels noirs en 
commun, on calculera S<17 - 2 x 6 = 17 - 12 = 5>.
...
Reprenons la comparaison entre la Cellule en ligne 1, colonne 38 et le Glyphe S<« l »> numéro 1.
...
S<Exception :> lors que l'on compare une Cellule avec le Glyphe-Espace, qui ne contient aucun
pixel noir, les neuf façons de faire varier les arrondis donneront toutes le même S<score :>
...
Il y a trois types de S<coloriage :>

=over 4

=item 1  Les coloriages servant  à détecter  le cas des  Cellules pour
lesquelles l'association hésite entre deux Glyphes, voire plus.

=item 2 Les coloriages servant à  mettre en évidence les Cellules avec
un très mauvais score.

=item  3 Les  coloriages servant  à faire  le tri  entre les  Cellules
associées  à  des  caractères   similaires  ;  typiquement,  faire  la
distinction entre  les S<« l  »> comme Lima,  les S<« I  »> majuscules
comme India, et les chiffres S<« 1 »>.
...
=for HTML
<img src="criteres-par-score.png" alt="Exemple de jeu de critères pour avoir les scores" />

Le résultat ressemble à S<ceci :>

=for HTML
<img src="resultat-scores.png" alt="Affichage d'un texte, les cellules étant coloriées selon leur score" />

On peut constater dans  cet exemple que le S<« e »> de  S<« begin »> a
...
un exemple de  coloriage pour faire la distinction entre  les S<« l »>
comme  Lima,  les  S<« I »>  majuscules  comme  India, et les chiffres
S<« 1 »>.

=for HTML
<img src="criteres-par-ressemblance.png" alt="Exemple de jeu de critères pour montrer les caractères similaires" />

Et voici le résultat

=for HTML
<img src="resultat-ressemblance.png" alt="Affichage d'un texte, les caractères similaires étant mis en relief" />

=head2 Modèle physique des données

S<Rappel :> le modèle logique des données S<est :>
...
Aucune des relations hiérarchiques (c'est-à-dire avec un C<(1,1)> d'un côté)
...
Champ C<_id> automatiquement créé par MongoDB
...
Comment interpréter les valeurs renvoyées par C<GD:getPixel>
...
Les différents états possible S<sont :>
...
Les informations de grille sont saisies et le fichier généré.

=item 3

La grille est validée, c'est-à-dire que les Cellules ont été
générées en fonction de cette grille.

=item 4

La procédure d'association a été lancée pour toutes les Cellules
du Document. Pour cet état, il y a deux horodatages, C<dh_debut> et C<dh_assoc>. Cela
permet de calculer le temps que cette fonction a pris pour le document
complet.

=item 5

Le fichier texte final a été généré.
...
et ainsi de site. Mais j'ai utilisé la valeur de l'attribut C<prio>
...
Clé alternative. Elle coïncide avec la clé C<car>, sauf pour les Glyphes associés à certains
caractères S<spéciaux :> espace, point, dollar et apostrophe.
Pour les espaces, on a la valeur C<"SP"> dans C<car1>, pour une meilleure lisibilité dans
...
Évidemment, la clé (pour identifier le Glyphe) comporte aussi le champ C<num>,
...
Caractéristiques du S<dessin :> dimensions et nombre de pixels noirs.
...
Caractéristiques de l'enveloppe des pixels noirs. Le champ C<data>
...
=for HTML
<img src="Cellule-p.png" alt="Dessin de la Cellule p en gros pixels, pour mettre en valeur les différentes coordonnées" />
...
les pixels se font de C<0> à C<lge - 1> et de C<0> à C<hte - 1>.
...
Glyphes. Le champ C<glyphes> contient les clés pour tous les Glyphes
sélectionnés, ainsi que les valeurs utilisées pour faire coïncider les centres de gravité.
Le champ C<cpt_car> contient l'histogramme par caractère
de C<glyphes> et le champ C<cpt_car> contient le nombre de paires stockées
dans le champ C<glyphes>.
...
un S<« r »> et un espace. Vous pouvez constater que dans le 
tableau C<glyphes> on utilise la clé normale de l'espace, 
tandis que dans le hachage C<cpt_car> on prend la clé 
affichable C<"SP">.
...
Les valeurs possibles de C<select> S<sont :>
...
En enchaînant plusieurs Critères avec des valeurs décroissantes de C<seuil>,
...
dépend d'un caractère stocké dans C<caract>. Si C<selspace> est
...
Critère précédent de type C<multiple>.
...
C<« nouveau »>, qui permet d'enchaîner vers la page des Coloriages,
mais pour créer un nouveau Coloriage au lieu d'afficher et de modifier
un Coloriage existant.

Adresse pour un Document C<exemple1> : L<http://localhost:3000/doc/exemple1>.

=head4 Variante : Grille

La page de base affiche le fichier PNG d'où est tiré le Document,
tandis que la variante S<« grille »> affiche un dessin où les 
bords des Cellules sont matérialisés.

Adresse pour un Document C<exemple1> : L<http://localhost:3000/grille/exemple1>.

Ici le mot S<« grille »> est au singulier, car j'ai choisi ce schéma
d'adresse avant la mauvaise surprise qui m'a conduit à isoler l'entité
logique Grille de l'entité Document.

=head4 Formulaire : grille
...
S<Fonctionnement :> le formulaire est constitué des I<S<n - 1>> premières
...
Le champ S<« Action »> prend les valeurs S<suivantes :>
...
Les valeurs de C<dx>, C<dy> et des cisaillements sont simplement reprises telles quelles
de la Grille englobante, sans modification. Quant aux valeurs de C<x0> et C<y0>, elles sont calculées 
...
J'ai utilisé l'abréviation S<« suppr »> au lieu du mot entier S<« suppression »>
...
et de laisser le champ Action à S<« rien »>. Dans ce cas, les valeurs saisies sont ignorées
et l'utilisateur (moi) se demande pourquoi la saisie n'a pas été prise en compte.

Un point particulier à signaler : même si l'action est S<« rien »>, la valeur saisie dans le
champ Priorité est prise en compte.

=head4 Formulaire : validation de la Grille

Ce deuxième formulaire ne comporte qu'un bouton d'action. Il permet
de valider le découpage, avec création des
S<« Cellules ».> Cela ne sert à rien de modifier les valeurs dans
le premier formulaire, elles sont ignorées si vous cliquez sur
S<« Validation »>.

=head4 Formulaire : Association des Cellules avec les Glyphes
...
=head4 Pseudo-formulaire : liste des Coloriages
...
=head4 Formulaire : Génération du fichier texte
...
=head3 Page : Coloriage

Cette page reçoit en paramètre le numéro d'un Coloriage ou le mot
C<"nouveau">. Dans le premier cas, c'est pour travailler sur un Coloriage
...
Adresse : L<http://localhost:3000/coloriage/exemple1/2> pour le deuxième
Coloriage du Document C<exemple1>, ou L<http://localhost:3000/coloriage/exemple1/nouveau>
pour créer un nouveau Coloriage pour ce même Document.

=head4 Formulaire : Suppression
...
Pourquoi six S<Critères ?> Parce qu'à l'origine, je n'avais pas prévu de donner le choix
...
Adresse : L<http://localhost:3000/cellule/exemple1/0/3> pour la Cellule en ligne
0 et en colonne 3 du Document C<exemple1>.

=head4 Variante : top 10

Cette page a une variante, S<« top10 »>, qui reprend la recherche du meilleur
Glyphe, mais ne s'arrête pas au meilleur d'entre eux. Il prend les dix
meilleurs et les affiche tous les dix.

Adresse : L<http://localhost:3000/top10/exemple1/0/3>

=head4 Formulaire : Création de Glyphe
...
=head4 Formulaire : Association aux Glyphes

Permet de lancer la recherche du meilleur Glyphe, comme le formulaire
S<« Association> des Cellules avec les S<Glyphes »> de la page S<« Document »>,
...
Reconstituer le quatrième fichier source, celui que j'ai renoncé à numériser
...
Prendre en compte les annotations sur le listing du 19 janvier et corriger les éventuelles
...
je n'utilise plus de tabulations en dehors des fichiers F<makefile>. Et
L<ça rapporte plus|https://arstechnica.com/information-technology/2017/06/according-to-statistics-programming-with-spaces-instead-of-tabs-makes-you-rich/>.
...
=head3 Mises à jour concurrentes
...
Un exemple S<flagrant :> certains formulaires reposent sur l'inclusion
de champs cachés C<hidden> pour mémoriser certaines valeurs sans permettre
...
c'est une grossière S<erreur !> Elle permet à un utilisateur averti de
...
et rapide plutôt qu'une solution évoluée, mais longue à mettre en oeuvre.
...
j'ai travaillé en I<air gap>. La machine où j'ai développé mon programme de reconnaissance
...
=head3 Méthodes de programmation à la mode
...
raison que j'ignore, les fichiers ont été stockés en niveaux de gris, en I<1-bit grayscale>. Certes, c'était
du I<1-bit>, mais c'était du I<grayscake>, avec du gris très foncé et du gris très clair, alors que
mon programme attendait du noir (RGB = 000000) et du blanc (RGB = FFFFFF), pas du
gris clair et du gris foncé. Heureusement, le programme C<conv-nb> que j'avais écrit pour le listing
du 10 janvier avec un ruban encreur fatigué a été capable de convertir également
ce listing  I<1-bit grayscale> en véritable noir et blanc.
...
en noir et blanc avec C<conv_nb>. Le résultat a été des fichiers PNG noir et blanc en 72 pixels
par pouce. Mais ce n'est pas un problème, parce que tous les pixels ont été conservés.
En fait, en prenant le fichier F<b001-coul.png>, il fait 2550 x 3509 pixels, soit
21 cm x 29,7 cm, tandis que le fichier converti F<b001_nb.png> a pour dimensions
900 mm x 1238 mm (plus d'un mètre de hauteur) soit 2550 x 3509 pixels.

=head3 GD

J'ai déjà mentionné le fait que la conversion RGB -E<gt> HSV pouvait donner des valeurs négatives
pour la teinte H. J'ai soumis un ticket sur RT et le module a été corrigé.
Mais j'ai constaté d'autres problèmes pour la conversion RGB -E<gt> HSV ou inverse. Quand j'aurai le
...
Cela dit, si j'avais I<RTFM> jusqu'au bout, j'aurais trouvé le
L<paragraphe|https://metacpan.org/pod/distribution/Dancer2/lib/Dancer2/Tutorial.pod#Serving-static-files>
...
stockés dans MongoDB. Dans mon cas, cela s'est produit avec l'histogramme C<cpt_car>
...
J'ai donc étendu la visualisation par digramme de l'espace à ces deux caractères C<PT> pour le
point et C<AP> pour l'apostrophe. Et bien que mon programme Pascal n'utilise aucun dollar (ou peso),
j'ai ajouté un digramme C<DL> pour ce caractère.

=head3 Le programme lui-même
...
problème avec les blancs insécables, les balises C<S> de POD ne sont pas prises en compte.
Un autre problème que j'ai découvert un an plus tard, lors de mon travail sur
L<l'As des As|https://github.com/jforget/Perl6-Alpha-As-des-As-Zero/blob/master/Description/description-fr.pod>.
À plusieurs endroits, j'ai mis deux images côte à côte, en prenant soin d'ajouter un attribut
C<width="45%"> pour chacune. Malgré cette précaution, les images apparaissent l'une au-dessus de l'autre
...
y  a eu  un problème  dans Github,  le convertisseur  POD -E<gt>  HTML
plantait. La version  POD du présent document était  donc impossible à
visualiser. Ensuite,  à l'été  2022, le  convertisseur a  recommencé à
fonctionner,  mais   avec  quelques  problèmes  d'encodage   pour  les
paragraphes  contenant   des  I<codepoints>  au-delà  de   255,  comme
C<U+0153>  (I<LATIN SMALL  LIGATURE  OE>)  ou C<U+2192>  (I<RIGHTWARDS
ARROW>). D'où la conversion en Markdown.

Il va de soi que pour écrire un programme de reconnaissance, il faut faire attention
aux caractères semblables, tels que les S<« l »> (S<« L »> minuscules),
les S<« I »> (S<« i »> majuscules) et les S<« 1 »> (chiffres S<« un »>).
...
destinés à détecter les confusions entre Glyphes similaires, notamment S<« l »> (S<« L »> minuscules),
S<« I »> (S<« i »> majuscules) et S<« 1 »> (chiffres S<« un »>). Pour ces Coloriages,
...
jusqu'à la liste des Coloriages. Puis afficher les Coloriages en sélectionnant S<« ouvrir>
dans un nouvel S<onglet »>, cela permet de revenir facilement et rapidement à la liste
des Coloriages de l'ancien Document.

Affichage d'une S<Cellule :> si cette Cellule est associée à un mauvais Glyphe, créer un
...
l'on consulte une Cellule, il faut commencer par cliquer sur le bouton S<« association »>.
...
et que le compilateur n'a pas trouvé le C<end.> mais on peut trouver d'autres erreurs de
compilation résultant d'une mauvaise association entre une Cellule et un Glyphe.
C'est ainsi que j'ai eu une erreur parce que le compilateur était tombé sur un C<hegin>
là où il attendait un C<begin>. J'ai donc corrigé cette Cellule en lui créant un Glyphe,
mais également j'ai créé un Coloriage mettant en relief les S<« b »> et les S<« h »>
...
de fortes probabilités de confusion entre le S<« j »> minuscule et le S<« J »> majuscule.
Et le compilateur Pascal ne voit aucun problème à appeler la même donnée 
C<date.jour> sur une ligne et C<date.Jour> sur une autre.
...
suppression jusqu'à la fin de ligne (C<d$> pour vi et pour Emacs-Viper).

=head2 Bilan

Mes trois listings Pascal ont à peine plus de 500 lignes, soit un total de S<1 500>. Mon programme Dancer2 + MongoDB
fait plus de S<1 900 lignes>, auxquelles il faut ajouter une centaine de lignes pour le programme
de conversion bleu -E<gt> noir + orange -E<gt> blanc. Et le programme Dancer2 + MongoDB n'est pas
maintenable s'il n'est pas accompagné du présent document, qui dépasse les 2500 lignes
(je ne compte pas les compléments ci-dessous). Écrire S<4 500 lignes> de Perl et de POD
pour s'éviter la saisie de S<1 500 lignes> de Pascal, est-ce S<rentable ?>
...
des formulaires HTML et des manipulations de pixels dans L<GD>. Certes, je ne peux
...
=head2 Mon environnement de développement
...
Lorsque je travaille sur ce projet, je commence par ouvrir une fenêtre C<xterm>
et je lance les deux S<commandes :>

  cd Documents/prog/Perl-fixed-width-char-human-recognition/
  emacs &

Et je laisse de côté la fenêtre C<xterm>, sauf de temps en temps pour
taper des commandes du S<genre :>
...
Lors d'un C<commit>, Git ouvre une fenêtre dans C<vi> (pas Vim)
...
J'utilise Emacs en mode C<viper>. Mais si vous préférez Emacs S<« à> la S<vanille »>,
...
en mode C<dired>. C'est plus une habitude qu'un besoin.

J'ouvre également une fenêtre d'édition sur le fichier source F<appli/appli.pl>
et une autre sur le fichier de documentation F<description/description.pod>.

J'ouvre une fenêtre shell avec C<M-x shell>. Dans cette fenêtre shell, je lance
Dancer2 en S<tapant :>

  cd fichiers
  perl ../appli/appli.pl

S<Rappel :> pour  simplifier les contrôles de sécurité, je n'autorise que des noms de fichiers
sans aucun slash, donc placés dans le répertoire courant. C'est pour cela que l'application
doit être lancée depuis le sous-répertoire F<fichiers>.

J'ouvre une deuxième fenêtre shell avec C<C-u M-x shell>, que je nomme S<« mongo »>.
Compte tenu de la façon dont fonctionne C<C-u> en mode viper, il faut être en mode
vi-insertion plutôt qu'en mode vi-commande lorsque l'on tape C<C-u M-x shell>.
Cela ne vous concerne pas si vous être en mode Emacs standard.
Et dans cette fenêtre, je S<tape  :>
...
et copier-coller le résultat vers le fichier F<description/description.pod>.
...
Et je demande L<http://localhost:3000> pour avoir la fenêtre de connexion.
...
S<« ouvrir> dans un nouvel S<onglet »>. La raison sera expliquée dans le paragraphe 
suivant.

=head3 Modification du programme

Chaque fois que je veux prendre en compte les modifications effectuées sur le
programme F<appli/appli.pl>, voici les actions effectuées.

=over 4

=item 1

Dans la fenêtre C<*Shell*> d'Emacs, je tape C<C-c C-c> (contrôle C deux fois) pour
...
Dans la même fenêtre, je remonte jusqu'à trouver une ligne de commande
...
Dans le premier onglet de Firefox, qui donne la liste des documents, 
j'utilise l'action S<« retour »> (Alt-flèche gauche) pour revenir à l'écran de connexion.

=item 4

Je retape le code appli et le mot de passe pour me connecter à la nouvelle
instance du serveur Dancer2.

=item 5

Dans les autres onglets de Firefox, je fais C<F5> pour réafficher la page 
...
Comme vous pouvez le constater en lisant le source de F<appli/appli.pl>,
je suis un adepte du débugage S<par :>
...
Peut-être devrais-je tenter d'utiliser l'outil C<perldb> d'Emacs...
D'un autre côté, en lisant la documentation Emacs de C<perldb>, je m'aperçois
